...
class Nut0nEnv(py_environment.PyEnvironment):
    def __init__(self):
        ...
        self.max_platforms = 13
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(13,2), dtype=np.float32, minimum=0, name='observation')
    ...
    def _step(self, action):
        ...
        #sleep(0)
        if self.player.alive == False:
            return ts.termination(self._get_observation(), reward=-20)
        else:
            return ts.transition(self._get_observation(), reward=self.seconds, discount=-10)
